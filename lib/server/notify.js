...
export async function notifyNewLead(lead) {
  ...
  const replySubject = encodeURIComponent(`Your ${lead.serviceType} quote — The Long Island Cleanout Company`);
  const replyBody = encodeURIComponent(
    `Hi ${lead.name},\n\nThanks for reaching out to The Long Island Cleanout Company about your ${lead.serviceType.toLowerCase()}${lead.address ? ` at ${lead.address}` : ""}.\n\n`
  );
  const mailto = lead.email ? `mailto:${lead.email}?subject=${replySubject}&body=${replyBody}` : "";
  const phoneCell = lead.phone
    ? `<a href="tel:${telDigits}" style="color:#0e1520;text-decoration:none;">${lead.phone}</a>`
    : "";
  const emailCell = mailto
    ? `<a href="${mailto}" style="color:#0e1520;text-decoration:none;">${lead.email}</a>`
    : "";
  ...
  await sendEmail({
    ...
    html: emailLayout(
      "New quote request",
      `<p style="margin:0 0 16px;color:#5b6672;">A new quote request just came in. Reply right from this email using the buttons below, or open the dashboard to send a price.</p>
       ...
       ${lead.phone ? emailButton(`sms:${telDigits}`, "Text customer") : ""}
       ${mailto ? emailButton(mailto, "Email customer") : ""}
